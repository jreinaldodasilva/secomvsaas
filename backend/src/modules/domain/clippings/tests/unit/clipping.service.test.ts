...
jest.mock('../../repositories/clipping.repository', () => ({
  ClippingRepository: jest.fn().mockImplementation(() => mockRepo),
}));

import { ClippingService } from '../../services/clipping.service';

describe('ClippingService', () => {
  let service: ClippingService;

  beforeEach(() => {
    jest.clearAllMocks();
    service = new ClippingService();
  });
  ...
  it('should create and emit event', async () => {
    mockRepo.create.mockResolvedValue({ _id: 'id1', title: 'Matéria sobre Secom' });
    const result = await runInTenant(() => service.create({ title: 'Matéria sobre Secom', source: 'G1' }));
