...
4. After implementation, perform the following validation steps:
   ...
   * Verify that the changes integrate correctly with the existing component library and design token system.
...
5. **Testing Requirements**
   ...
   * Modifies existing prop interfaces or variant logic
   * Changes validation feedback or error state rendering
   * Fixes a component bug that could regress in the future
   ...
   * Replacing hardcoded values with `var(--token-name)` in CSS Modules with no behavioral change
...
* Preserve the design token system: all style values must use `var(--token-name)` from `src/styles/tokens/index.css` — do not introduce hardcoded color, spacing, radius, or shadow values.
* Preserve CSS Module scoping — do not move component styles into `global.css` unless the Quick Win explicitly requires it.
