...
4. After implementation, perform the following validation steps:
   ...
   * Verify that the changes integrate correctly with the existing routing and authorization architecture.
...
5. **Testing Requirements**
   ...
   * Introduces new routing logic or guard behavior
   * Modifies redirect logic or `state.from` handling
   * Changes session management behavior
   * Alters component behavior or rendered output
   * Fixes a navigation bug that could regress in the future

   In these cases:

   * Add **minimal, targeted tests** aligned with the project's current testing strategy (Vitest + React Testing Library).
   * Ensure the tests validate the intended navigation behavior change.

   **Tests are NOT required when the Quick Win is a small, low-risk change**, such as:

   * Documentation updates or inline code comments
   * Naming improvements
   * Dead code removal (unused layout components)
   * Non-functional refactoring
   * Minor structural adjustments with no behavioral change (e.g., `Link` → `NavLink` styling, sidebar persistence)
...
* Preserve the established layering: HTTP client → services → domain hooks → pages.
* Preserve **RBAC enforcement** via `ProtectedRoute` and `PermissionGate` — do not weaken or bypass existing guards.
...
* Explanation of what navigation behavior they validate
