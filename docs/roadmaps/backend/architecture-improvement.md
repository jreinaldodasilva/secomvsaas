...
| # | Issue | Architectural Impact | System Area | Effort | Dependencies | Source Section | Status |
|---|---|---|---|---|---|---|---|
| P1-1 | `DashboardService` (platform layer) directly imports repositories from all 7 domain modules | Inverts the intended dependency direction: platform → domain. Prevents independent module extraction and creates a fan-in coupling point | Layering / Service Boundaries | 3–5 days | All 7 domain repositories | Part 1 §2.6, Part 2 §6.5, Part 3 §8.2 | ⏳ Open |
...
| P2-5 | `ensureDefaultTenant()` runs a MongoDB query on every server cold start | Negligible at current scale; becomes a latency concern in high-frequency restart scenarios (e.g., rolling deploys, crash loops) | Bootstrap / Performance | 0.5 days | `src/seeds/defaultTenant.ts` | Part 2 §4.7 | ⏳ Open |
...
|---|---|---|---|---|---|
| Structural Layering Debt | `DashboardService` in platform layer imports all 7 domain repositories; `src/controllers/` naming ambiguity | `src/controllers/` ambiguity resolved (QW-7) | `DashboardService` dependency inversion prevents module extraction | 3–5 days | P1 |
...
| Data Scoping Debt | Citizen role lacks row-level `userId` filtering; `tenantScopedMixin.ts` shim was an incomplete refactoring | Shim deleted (QW-6) | Citizen data exposure risk remains — the only open P0 issue | 3–5 days | P0 |
...
| Deployment Architecture Debt | Single-process model obscures HTTP/worker boundary; no migration discipline; static AWS credentials in env | Worker dependency enforced (QW-9); static credential warning added (QW-4); text index migration added (QW-8) | HTTP/worker process separation model undocumented; migration discipline not yet established | 4–6 days | P1 |
...
| Issues resolved | 0 / 14 | **9 / 14** (QW-1 through QW-9) |
| Open P0 issues | 2 | **1** (P0-3 only) |
| Open P1 issues | 5 | **3** (P1-1, P1-4, P1-5) |
| Open P2 issues | 8 | **4** (P2-5, P2-6, P2-7 partial, P2-8) |
...
| P0-3 | Add `userId` filter enforcement in citizen-facing service methods for `appointments` and `citizen-portal` | 3–5 days | ⏳ Pending |
...
| Issue | Task | Effort | Status |
|---|---|---|---|
| P1-1 | Move `DashboardService` to `src/modules/dashboard/` or refactor to consume domain events/read models instead of direct repository imports | 3–5 days | ⏳ Pending |
...
**Remaining:** 5–8 days (P1-1, P2-6, P2-8)
...
| P1-4 | Document and validate the HTTP/worker process separation model; add Docker Compose `depends_on` or process supervisor configuration | 2–3 days | ⏳ Pending |
| P2-5 | Add startup flag (e.g., Redis key or env var) to skip `ensureDefaultTenant()` after first successful run | 0.5 days | ⏳ Pending |

**Total Phase 3 effort:** 10.5–16.5 days
**Remaining:** 7.5–11.5 days (P3-4, P3-5, P1-4, P2-5)
...
| Citizen data scoping enforcement | Not enforced at service layer | Not enforced at service layer | 100% `userId`-filtered for citizen role | Code audit of citizen-facing service methods |
| Platform → domain dependency violations | 1 (`DashboardService` imports 7 repos) | 1 | 0 | Dependency graph analysis (e.g., `madge`) |
...
| Worker process isolation | Not enforced in development | **Enforced via `dev:all` script + Redis health signal** | Enforced via Docker Compose or process supervisor | Local dev startup validation |
...
> Current score: **77 / 100** (post-quick-wins)

| Dimension | Baseline | Current | Max | Notes |
|---|---|---|---|---|
| Layering Discipline | 14 | **15** | 20 | `src/controllers/` naming ambiguity resolved (QW-7); `DashboardService` dependency inversion remains |
| Modularity | 16 | **18** | 20 | `tenantScopedMixin.ts` shim deleted (QW-6); no DI container remains |
| Scalability Readiness | 10 | **12** | 15 | `$text` indexes added for press-releases and clippings (QW-8); real-time layer still absent |
...
| Deployment Maturity | 7 | **9** | 10 | Worker startup enforced via `dev:all` + Redis health signal (QW-9); rate limiter fully env-configured (QW-5) |
| Tenant Scoping Robustness | 7 | 7 | 10 | No change; citizen row-level gap remains |

**Baseline: 70 / 100 → Current: 81 / 100**
...
1. **Citizen data exposure (P0-3) — the only remaining P0 issue:** The `citizen` role has `appointments:write` and `citizen-portal:read` permissions, but the service layer does not enforce `userId` filtering. A citizen can potentially read or modify other citizens' records. This is a data privacy risk that must be addressed before the citizen portal is publicly accessible.

2. **Dependency inversion (P1-1):** `DashboardService` in the platform layer directly imports repositories from all 7 domain modules. This is the single largest structural violation in the codebase — it prevents independent module extraction and creates a maintenance bottleneck as the domain grows.

3. **Minimal domain test coverage (P1-5):** Five of seven domain modules still have no unit or integration tests (repository tests were added for press-releases and clippings in QW-8). Business logic regressions in services are undetected. This is the highest-risk gap for production stability as the system evolves.
...
1. **Complete Phase 1 immediately** — only P0-3 (citizen `userId` scoping) remains. This is the sole blocker to unconditional production readiness for the citizen portal.
