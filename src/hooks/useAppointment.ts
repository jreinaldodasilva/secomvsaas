...
export function useAppointmentList(params?: Record<string, string | number | boolean | undefined>) {
  return useApiQuery<ApiResult<PaginatedData<any>>>(KEYS.all as unknown as string[], '/api/v1/appointments', params);
}

export function useAppointmentDetail(id: string) {
  return useApiQuery<ApiResult<any>>(KEYS.detail(id) as unknown as string[], `/api/v1/appointments/${id}`, undefined, { enabled: !!id });
}

export function useCreateAppointment() {
  return useApiMutation<ApiResult<any>, Record<string, unknown>>('post', '/api/v1/appointments', {
