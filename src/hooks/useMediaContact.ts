...
export function useMediaContactList(params?: Record<string, string | number | boolean | undefined>) {
  return useApiQuery<ApiResult<PaginatedData<any>>>(KEYS.all as unknown as string[], '/api/v1/media-contacts', params);
}

export function useMediaContactDetail(id: string) {
  return useApiQuery<ApiResult<any>>(KEYS.detail(id) as unknown as string[], `/api/v1/media-contacts/${id}`, undefined, { enabled: !!id });
}

export function useCreateMediaContact() {
  return useApiMutation<ApiResult<any>, Record<string, unknown>>('post', '/api/v1/media-contacts', {
